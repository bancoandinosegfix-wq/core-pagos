...
const LINEA = '0290001110000000012345' + 'CR' + '000000012500000' + 'REF-SPI-000012  ';

test('parsea una línea del lote de la cámara', () => {
    const [m] = parsearLote(LINEA);
    assert.strictEqual(m.cuenta, '0290001110000000012345');
    assert.strictEqual(m.tipo, 'CR');
    assert.strictEqual(m.monto, 125000);
    assert.strictEqual(m.referencia, 'REF-SPI-000012');
});
